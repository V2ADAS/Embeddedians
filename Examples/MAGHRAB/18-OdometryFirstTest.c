...
int main (void){
	...
	//HSERVO_vServoDeg(SERVO1, (s8)(-servo_angle));
//	MSYSTICK_vDelayms(2000);

	f32 distance = 59.5 ;
	CarControl_Move(FORWARD, 50, -40, 70) ;
//	CarControl_Move(Direction, Distance, Steering, speed)
	...
	while(1){
		...
		//yaw_rad = ( (yaw_mpu * PI) / 180.0 ) ;
		delta_d += get_delta_distance() * 0.85 ;
...
//			y +=  cos(yaw_rad)*delta_d / 1000;
			Update_Odometry(delta_d/1000 , yaw_mpu );
			delta_d=0;
		}
		if (distance - s < 15 ) {
//			x +=  sin(yaw_rad)*delta_d / 1000;
